...
@section('title')新建文章@stop


@section('content')

	<div class="col-md-8">
	<form action="{{ route('admin_store_post') }}" method="post">
		{{csrf_field()}}
		<div class="form-group">
			<label>title</label>
			<input type="text" class="form-control" name="title" placeholder="title" value="{{old('title')}}" required="required">
		</div>
		<div class="form-group">
			<label>slug</label>
		...
		<div class="form-group">
			<label class="radio-inline"><input type="radio" name="status" value="public" checked="checked"> 公开 </label>
			<label class="radio-inline"><input type="radio" name="status" value="hidden"> 隐藏 </label>
		</div>
		<div class="form-group">
			<label class="radio-inline"><input type="radio" name="type" value="post" checked="checked"> 文章 </label>
			<label class="radio-inline"><input type="radio" name="type" value="page"> 页面 </label>
		</div>
		<div class="form-group">
			<label>选择分类</label>
		...
		<div class="form-group">
			<label>内容</label>
			<textarea class="form-control" rows="30" name="content">{{old('content')}}</textarea>
		</div>
		{{-- 标签 start --}}
		<div class="form-group">
			<label>选择标签</label>
